...
export default async function ToolOutputPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const lines: string[] = [output.title, typeLabels[output.type] ?? output.type, ""];

  if (output.type === "rubrica") {
  ...
  } else if (output.type === "realworld") {
    const rw = c as unknown as RealWorldContent;
    if (rw.hook) lines.push(`"${rw.hook}"`, "");
    if (rw.examples?.length) { lines.push("EJEMPLOS DEL MUNDO REAL"); rw.examples.forEach((e) => lines.push(`▸ ${e.title}`, e.description, "")); }
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <ActionButtons
            ...
            filename={`${output.title.replace(/\s+/g, "_")}.txt`}
